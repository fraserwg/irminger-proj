...
if int_wind_stress:
    ...
    axs[2].set_ylabel("$\\partial_t\\mathcal{V}$ ($\\times$10$^{-2}$ Sv$\\,$km$^{-1}$)")
    axs[4].set_xlabel("Integrated wind stress ($\\times$10$^{5}$ N$\\,$s$\\,$m$^{-2}$)")
...
if tau_int_delta_mld:
    ...
    ax.set_xlabel("Integrated wind stress ($\\times$10$^{5}$ N$\\,$s$\\,$m$^{-2}$)")
    ax.set_ylabel("$\Delta$ MLD (m)")
    ax.set_title("Change in mixed layer depth")
    ax.legend(title="Wind duration (days)")
